...
def sanitize_name(name: str) -> str:
    ...
    ----------
    name : str
        The model or prompt name to sanitize.

    Returns
    -------
    str
        Sanitized name suitable for filesystem usage.
    """
    if not name:
        return "unknown"
...
def get_effective_prompt_style(args, config_file: str = "workflow_config.json") -> str:
    ...
    -------
    str
        The effective prompt style name to use.
    """
    # Command line argument takes precedence
...
class WorkflowOrchestrator:
    ...
    Coordinates all steps of the workflow: video frame extraction, image conversion,
    AI-powered image description, and HTML report generation.
    ...
    statistics : dict
        Dictionary for workflow statistics.
    """
    ...
    def __init__(self, config_file: str = "workflow_config.json", base_output_dir: Optional[Path] = None):
        ...
        self.base_output_dir = base_output_dir or self.config.base_output_dir
        self.logger = WorkflowLogger(base_output_dir=self.base_output_dir)
        self.statistics = {}
        if base_output_dir:
