...
class VideoFrameExtractor:
    """
    Extracts frames from video files using time interval or scene change detection.
    ...
    statistics : dict
        Stores statistics about extraction process.
    ...
    create_default_config_file(config_file: str) -> dict
        Create a default configuration file and return the config.
    calculate_scene_change(frame1: np.ndarray, frame2: np.ndarray) -> float
        Calculate the difference between two frames as a percentage.
    extract_frames_time_interval(video_path: str, output_dir: str) -> list[str]
        Extract frames at regular time intervals.
    extract_frames_scene_change(video_path: str, output_dir: str) -> list[str]
        Extract frames when scene changes are detected.
    resize_frame(frame: np.ndarray) -> np.ndarray
        Resize frame based on config settings.
    process_video(video_path: str) -> list[str]
        Process a single video file.
    log_final_summary() -> None
        Log comprehensive final statistics.
    find_video_files(directory: str) -> list[str]
        Find all video files in directory and subdirectories.
    run(input_path: str) -> None
        Main execution method.

    """
    def __init__(self, config_file: str = "video_frame_extractor_config.json", log_dir: str = None):
        self.supported_formats: set[str] = {'.mp4', '.avi', '.mkv', '.mov', '.wmv', '.flv', '.webm', '.m4v'}
        self.config_file: str = config_file  # Store for logging
        self.log_dir: str = log_dir
        self.setup_logging()  # Set up logging first
        self.config: dict = self.load_config(config_file)  # Now we can use logger in load_config
...
def get_default_config(self) -> dict:
    ...
    def calculate_scene_change(self, frame1: np.ndarray, frame2: np.ndarray) -> float:
        ...
        -------
        float
            Percentage of changed pixels between frames.
        """
